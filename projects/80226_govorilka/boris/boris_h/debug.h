...
#define debug_failed bDebug("failed")

#ifdef CONSOLE_DEBUG

#define bDebug(T) qDebug()<<"debug" << __FILE__<< ", " << __LINE__ << "(" << __FUNCTION__ << "): " << T
...
#define debug_show_and_die(T) {bDebug(T);qApp->closeAllWindows();QMessageBox::critical(0, "debug_show_and_die", currentSrcCodePosition + QString("%1").arg(T));qApp->exit(-1);}
